...
public class QuotaAmendController extends  BaseController {

    @GetMapping("/list")
    public ResponseEntity<PageResult<QuotaAmend>> queryDifferenceList(
        ...
        PageResult<QuotaAmend> differenceServicePageResult = quotaAmendService.pageQueryList(pageNum, pageSize, keywords);
        differenceServicePageResult.setPageNum(pageNum);
        return ResponseEntity.ok(differenceServicePageResult);
    }

    @PostMapping("/add")
    public ResponseEntity<Void> saveDifference(@RequestBody QuotaAmend quotaAmend){

        quotaAmendService.save(quotaAmend);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
    @PutMapping("/update")
    public ResponseEntity<Void> updateDifference(@RequestBody QuotaAmend quotaAmend){


        quotaAmendService.update(quotaAmend);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
    ...
    public ResponseEntity<Void> uploadFileExcel(@RequestParam("file") MultipartFile multipartFile) throws IOException {
        ...
        quotaAmendService.upload(multipartFile);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
    ...
    public ResponseEntity<Void> downloadFileExcel(HttpServletResponse httpServletResponse) throws Exception {
       /* try {
            ...
            List<QuotaAmend> all = quotaAmendService.findAll();
    ...
    public ResponseEntity<List<Variable>> selectOptions(){
        return ResponseEntity.ok(quotaAmendService.findAll());
    }
